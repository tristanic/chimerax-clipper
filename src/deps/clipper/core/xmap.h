...
namespace clipper
{
  ...
  public:
    ...
    std::vector<size_t> asu_indices; //!< Indices into the main data array corresponding to the unique ASU
  ...
  */
  ...
  public:
    ...
    protected:
      ...
      void set_asu_index(const size_t& i)
      {
          index_ = map_->cacheref.data().asu_indices[i];
      }
    };
    ...
    */
    ...
    { map_ = &map; index_=0; next(); }
      ...
      //! Constructor: takes parent map and desired index in the unique ASU
      Map_reference_index( const Xmap_base& map, const size_t& asu_index) { map_ = &map; set_asu_index(asu_index); }
    ...
    //if ( false ) {
      ...
      for (int i=0; i<num_threads; ++i)
      {
          thread_results.push_back(std::async(std::launch::async,
              [start, reflections_per_thread](const H& fphidata,
                  const Spacegroup& spacegroup, FFTmap_sparse_p1_hx& fftmap,
                  const Isymop* isymop)
              {
                  auto ih = typename H::HKL_reference_index(fphidata.hkl_info(), start);
                  for (int j=0; j<reflections_per_thread && !ih.last(); ih.next(), ++j)
                  {
                      const auto & fphi = fphidata[ih];
                      if (fphi.missing() || fphi.f() == 0.0)
                        continue;
                      ffttype f = fphi.f();
                      ffttype phi0 = fphi.phi();
                      const HKL& hkl = ih.hkl();
                      fftmap.set_hkl( hkl,
                          std::complex<ffttype>(f*cos(phi0), f*sin(phi0)));
                      for (int sym=1; sym < spacegroup.num_primops(); ++sym)
                      {
                          ffttype phi1 = phi0 + hkl.sym_phase_shift(spacegroup.symop(sym));
                          fftmap.set_hkl(hkl.transform(isymop[sym]),
                              std::complex<ffttype>(f*cos(phi1), f*sin(phi1)));
                      }
                  }
              },
              fphidata, spacegroup_, std::ref(fftmap), isymop
          ));
      ...
      {
          thread_results.push_back(std::async(std::launch::async,
              [start, points_per_thread](const Xmap<T>& self, FFTmap_sparse_p1_hx& fftmap)
              {
                  auto ix = Xmap_base::Map_reference_index(self, start);
                  for (size_t j=0; j<points_per_thread && !ix.last(); ix.next(), ++j) {
                      fftmap.require_real_data( ix.coord() );
                  }
              },
              std::cref(*this), std::ref(fftmap)
          ));
      ...
      {
          thread_results.push_back(std::async(std::launch::async,
              [start, points_per_thread](Xmap<T>& self, const FFTmap_sparse_p1_hx& fftmap)
              {
                  auto ix = Xmap_base::Map_reference_index(self, start);
                  for (size_t j=0; j<points_per_thread && !ix.last(); ix.next(), ++j) {
                      self[ix] = fftmap.real_data(ix.coord());
                  }
              },
              std::ref(*this), std::cref(fftmap)
          ));
          start += points_per_thread;
      }
      for (auto &r: thread_results)
    ...
    if ( type == Sparse || ( type == Default && default_type() == Sparse ) ) {
      ...
      for (size_t i=0; i<num_threads; ++i)
      {
          thread_results.push_back(std::async(std::launch::async,
              ...
              {
                  ffttype f;
                  Xmap_base::Map_reference_index ix(self, start);
